...
namespace CCmall.Repository
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        public async Task<PageModel<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression, string strOrderByFileds, bool Isdesc = true, int intPageIndex = 1, int intPageSize = 20)
        {
            if (Isdesc)
            {
                strOrderByFileds += " asc";
            }
            else
            {
                strOrderByFileds += " desc";
            }
            return await QueryPage(whereExpression, intPageIndex, intPageSize, strOrderByFileds);
        }
        /// <summary> 
        ///查询-多表查询
        /// </summary> 
